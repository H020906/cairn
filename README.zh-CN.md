...
| NaN 的载荷位改变不了答案 | **在四个引擎上（含一个 JIT 和浏览器自己的引擎）、300 个随机生成的浮点表达式上确认。** 而且是拿它真正针对的那个引擎验的牙口：删掉 `copysign` 逃逸点，**V8 给出 `-1.5`，而 Cairn 给出 `+1.5`** |
| 志愿者真正在跑的那个引擎，和 Cairn 一致 | **已确认，而在此之前它从没被检查过。** 1,046 个生成单元，经浏览器 worker 自己的 `host.js` 跑在 V8 上——输出、陷阱行为、精确指令数全对。测试里其他引擎都是链进同一个二进制的 Rust，没有一个是这个项目真正面向的那个 |
| 负载可以从宿主拿 `exp` 和 `sin` | **已推翻，而且是被两个不同的理由各推翻一次。** 两万个输入、十二个函数：V8 和平台 libm 返回的**位没有一个函数是全一致的**——`cbrt` 有 29.8% 的调用不一致，`sinh` 17.8%。光这一条就足以让两个诚实的志愿者站到争议的两边。更糟的是：对这个格式里最难的那个参数，平台自己的 `sin` 返回 **`-0.2227`，而正确答案是 `1.0`**——用 3000 位精度的 `π` 做精确整数运算、V8、以及本项目的库，三方一致地反对它。宿主的数学不只是"各家不一致"，而是"哪一家都靠不住"。见 [ADR-0016](docs/adr/0016-math-belongs-in-the-module-not-the-host.md) |
| 解释器在任意代码上与真实引擎一致 | **还不能这么说。** 整模块生成第一次跑就找出一个 bug——函数作用域下的 `br 0` 是返回，而 Cairn 没有函数标签。已修；146 个生成模块现在在 Cairn、wasmi、wasmtime 和 V8 上一致。目前只是"没有反证" |
...
| 原生 worker | **Rust** | 单个二进制、多核，给捐出来的机器用 |
| 记录系统 | **一份只追加的日志** | 单元、结果、争议。PostgreSQL 是被推迟而不是被计划：协调器没有任何查询要服务，每一次读都是在扫一个 `Vec`——见 [ADR-0014](docs/adr/0014-the-coordinator-keeps-a-log-not-a-database.md) |
...
志愿者会用上它能腾出来的每一个核。**用几个核是算出来的，不是问出来的**——`--jobs` 只能往
*小*调。原因是「一次跑几个」这个问题，随口给一个像模像样的答案太容易了：一个负载最多可以
声明 256 MiB，所以在一台 32 线程的笔记本上敲 `--jobs 32`，等于向一台别人还在用的机器要
8 GiB。这里的并发是被一池**字节**卡住的，而不是被线程数卡住的，所以一个同时跑大小负载的
网格不需要重新规划；而且预算里留了四分之一给真正吃内存的那件事——**争论**一个单元，那要
握住多达 32 份机器状态的克隆。

在写这段代码的笔记本上（i5-13500H，4 个性能核 + 8 个能效核，共 16 个硬件线程），跑
`workloads/examples/busy-loop.wat` 的 400 个单元：**单线程 56.0 秒，15 线程 7.7 秒**——
7.2 倍，不是 15 倍。**曲线在 4 这个点上拐弯，而 4 正好是性能核的个数**：到 4 为止效率还有
93%，再往后每加一条线程只值前面那些的三分之一左右。

志愿者会打印每个单元自己的执行耗时，差额去哪了一看便知：单独跑一个单元 136 毫秒，十五个
一起跑时变成 272 毫秒，也就是每条线程只干出了独自运行时 49% 的活。机器自身的上限因此在
7.4 倍左右，而 Cairn 拿到了其中的 96%。**捐出来的算力跟着物理硅片和散热余量走，而不是跟着
操作系统报出的核数走**——在一颗混合架构的笔记本 CPU 上，这两个数差了将近一倍。见
...
319 个测试，另外 `node --test browser/policy.test.js` 还有 12 个。其中包括：一个逐指令对照
...
**存在的是验证内核、两个 worker、一个把它们串成系统的协调器、跑在其上的交互式争议协议、一份杀掉进程也
不丢的日志，以及不用等第二个志愿者就能抓到作弊的诱饵任务。** 不在这里的：惩罚机制、面板，以及真实科学
负载。这一点被直说出来，而不是靠没打勾的复选框暗示。

| 里程碑 | 状态 |
|---|---|
| 仓库、CI、架构决策记录 | **完成**——CI 跑的是真正的确定性门禁，不是占位符 |
| **确定性执行内核 + 轨迹承诺** | **完成**——约 18,000 行 Rust 源码，319 个测试 |
...
| **一个用满整台机器的志愿者** | **完成**——并行度由内存算出来而不是配出来；在一台 16 线程笔记本上 7.2×，也就是这台机器能给出的 96% |
| **把浏览器引擎放进确定性测试网** | **完成**——1,046 个单元经志愿者自己的 host 跑在 V8 上。一验牙口就抓到一处真实的 NaN 符号位分歧 |
| **协调器持久化** | **完成**——一份只追加的日志，不是数据库。跑到一半直接杀掉再重启：60 个单元，执行 60 次，不丢不重 |
| **诱饵任务 + 信誉** | **完成**——第一个不用等第二个志愿者就能抓到作弊的机制。实测：常见作弊率下 3–21 个单元被抓，而按 1% 频率作弊的通常抓不到 |
| **每条抓到错误答案的路径现在都会上报** | **完成**——两方都无法举证时由裁判自己重跑该单元，于是它准确地知道谁错了；此前它是用一句英文散文说的，没有任何东西读得懂。记为"答案错误"，绝不记为"撒谎"——因为走这条路的志愿者就是浏览器 |
| 协调器：心跳、日志压实 | 未开始——日志目前只会变长 |
| 惩罚 | 刻意未做——被抓到的志愿者只是被查得更严，不做别的。把人排除出去需要的是运营者，不是一个常数 |
| **给负载用的确定性数学库** | **完成**——`cairn-math`，26 个函数，零依赖，只用 WebAssembly 精确规定了结果的那些运算搭出来。每个函数 20 万个样本，与平台相差 1–2 个最低位；在四个引擎上给出**完全相同的位**。这个格式里最难的那个参数，平台算错了，它算对了 |
| **编译器真正会发出的那套指令** | **完成，而且发现的东西和预想的不一样。** 原计划以为这一项是"补实现指令"。实际发现的是：`rustc` 把 `call_indirect` 的表索引写成填充过的五字节 LEB128，而规范在那个位置只允许一个零字节——于是**任何含有 trait object 或函数指针的负载都被拒了**，理由是"零"的拼写方式，而拒它的那道门背后的解释器早就完整实现了这条指令。见 [ADR-0018](docs/adr/0018-a-compilers-call-indirect-is-not-the-specifications.md) |
| **负载 SDK** | **完成**——复制 `workloads/template`,写你的 `run`,`cargo build --release`。作者原本要做对的七件事变成了一件,而那七件里每一件出错时,报的错都指向别的地方。`cairn-worker check` 回答"这个会不会被接受",并且给出修法而不只是规则。见 [ADR-0019](docs/adr/0019-no-std-is-a-proxy-and-the-import-list-is-the-property.md) |
| 面板 + 实时地球仪 | 未开始 |
| **一个真实的科学负载** | **完成——Lomb–Scargle 周期图**，在不等间隔的天文时间序列里找周期信号的标准工具，也正是 Einstein@Home 那类搜索的形状。一个单元 = 一个频带。它的测试做了这里其它测试都没做的事：除了要求三个引擎逐位一致，还合成一个已知频率的信号、要求峰值回到那个频率上——因为三个引擎算出同一个错答案时，它们也是完全一致的。**不是分子对接**——那是这一行写了一年的东西：对接的单元不是自足的（受体有几万个原子，而且每个单元都一样），而且对接分数除了它自己之外没有任何东西可以对照。见 [ADR-0020](docs/adr/0020-the-first-real-workload-is-a-periodogram-not-docking.md) |
